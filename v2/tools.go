...
const randomStringSource = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTVWXYZ0123456789_+"
			...
			uploadedFiles, err = func(uploadedFiles []*UploadedFile) ([]*UploadedFile, error) {
				...
				if renameFile {
					uploadedFile.NewFileName = fmt.Sprintf(
						"%s%s", t.RandomString(25), filepath.Ext(hdr.Filename))
				} else {
...
func (t *Tools) DownloadStaticFile(w http.ResponseWriter, r *http.Request, pathName, displayName string) {
	w.Header().Set("Content-Disposition", fmt.Sprintf("attachments; filename=\"%s\"", displayName))

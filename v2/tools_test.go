...
func TestTools_UploadFiles(t *testing.T) {
	for _, e := range uploadTests {
		...
		if !e.errorExpected {
			if _, err := os.Stat(
				fmt.Sprintf("./testdata/uploads/%s", uploadedFiles[0].NewFileName)); os.IsNotExist(err) {
...
func TestTools_UploadOneFile(t *testing.T) {
	...
	if _, err := os.Stat(
		fmt.Sprintf("./testdata/uploads/%s", uploadedFiles.NewFileName)); os.IsNotExist(err) {
...
var slugTests = []struct {
	...
	{name: "japanese string and roman characters", s: "hello こんにちは世界 world", expected: "hello-world", errorExpected: false},
}
...
func TestTools_DownloadStaticFile(t *testing.T) {
	...
	if res.Header["Content-Disposition"][0] != "attachments; filename=\"puppy.jpg\"" {
...
var jsonTests = []struct {
	...
	{name: "file too large", json: `{"foo": 1"}`, errorExpected: true, maxSize: 1, allowUnknown: true},
